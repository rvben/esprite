# CLI, native window and QEMU reference

For an interactive, iOS-Simulator-style view, run `serve` with `--window`:

```bash
esprite serve --target cyd_tft --window
```
...
board-declared key (waveshare_amoled_18: **space** = PRIMARY, **tab** =
SECONDARY,
...
serve [--window] [--scale N]     boot and keep pumping for a live bridge; --window
                                 opens an interactive SDL window
...
```json
{
  ...
  "steps": [
    ...
    { "cmd": "snapshot",
      "data": {"lim":1,"s5":42,"s5r":180,"s7":10,"s7r":6000} },
...
output fields, error kinds, exit codes). Commands default to JSON when piped;
logs go to
stderr.
...
## What each backend is observable for
...
|---|---|---|---|
| App logic, UI rendering, data parsing | observable | runs, observable via serial | runs, observable via display |
| RTOS scheduling, heap/stack pressure, watchdogs | not visible | observable | observable |
...
| Determinism | virtual clock; host I/O may vary | ESP32-C3: byte-exact serial across runs; ESP32/S3: wall-clock, load-sensitive | same per architecture |
